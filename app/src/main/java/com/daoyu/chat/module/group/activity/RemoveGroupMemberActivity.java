 ...
 * 移除群成员
 */
public class RemoveGroupMemberActivity extends BaseActivity implements CreateGroupChatAdapter.OnItemClickListener, OnItemClickListener {
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_complete:
                ...
                requestRemoveGroupChat(friendId, createIds);
                break;
        }
    }
        ...
        CHttpUtils.getInstance().requestDataFromServer(UrlConfig.URL_CONTACT_FRIEND_LIST, params, this, new JsonCallback<ContactFriendBean>(ContactFriendBean.class) {
            ...
            public void onSuccess(Response<ContactFriendBean> response) {
                ...
                if (contactFriendList != null && contactFriendList.size() > 0) {
                    ...
                    for (int i = 0; i < contactFriendList.size(); i++) {
                        ContactFriendBean.ContactFriendData contactFriendData = contactFriendList.get(i);
                        String userFriendId = String.valueOf(contactFriendData.userFriendId);
                        for (int j = 0; j < userHeaders.size(); j++) {
                            UsersHeaderBean.UsersHeaderData usersHeaderData = userHeaders.get(j);
                            if (userFriendId.equals(usersHeaderData.userId)) {
                    ...
                    if (positions.size() > 0) {
                        for (int i = 0; i < positions.size(); i++) {
                            ...
                            contactFriendList.add(contactFriendData);
                        }
                    }
    ...
    private void requestRemoveGroupChat(String firendID, String[] createIds) {
        showLoading("请稍后...", false);
        Map<String, Object> params = new HashMap<>();
        params.put("friends", firendID);
        params.put("group_id", groupInfoData.group_id);
        CHttpUtils.getInstance().requestDataFromServer(UrlConfig.URL_REMOVE_CHAT_FROUP, params, this, new JsonCallback<GroupInfoBean>(GroupInfoBean.class) {
            @Override
