...
import com.daoyu.chat.module.system.dialog.DeleteAddressDialog;
 ...
 * 编辑收货地址
 */
public class ModifyAddressActivity extends BaseTitleActivity implements ChooseProvinceCityAreaDialog.IChooseProvinceCityAreaListener, DeleteAddressDialog.ICheckSequenceListener {
    ...
    private void showDeleteAddressDialog() {
        if (deleteAddressDialog != null) {
            deleteAddressDialog.dismissAllowingStateLoss();
        }
        deleteAddressDialog = DeleteAddressDialog.getInstance();
        if (deleteAddressDialog.isAdded()) {
            deleteAddressDialog.dismissAllowingStateLoss();
        } else {
            deleteAddressDialog.show(getSupportFragmentManager(), "deleteAddressDialog");
        }
    }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.tv_save:
                ...
                String addressStr = editAddressDetail.getText().toString();
                if (TextUtils.isEmpty(addressStr)) {
                    toast.toastShow("请填写详细地址");
                    return;
                }
                String longAddress = area + "=+=" + addressStr;
    ...
    private void requestDeleteAddress(ShippingAddressData address) {
        UserBean.UserData userInfoData = BaseApplication.getInstance().getUserInfoData();
        Map<String, Object> params = new HashMap<>();
        params.put("uId", userInfoData.uid);
